...
// size of grid
static const size_t GRIDSIZE = 3;
    ...
    for(size_t ie=0; ie<mesh.GetNumElements(); ie++) {
        const FEMElementTri& elem = mesh.GetElement(ie);
         double A=0.;
          ...
          Vector2 l1=mesh.GetNodePosition(elem.GetGlobalNodeForElementNode(1))-mesh.GetNodePosition(elem.GetGlobalNodeForElementNode(0));
          Vector2 l2=mesh.GetNodePosition(elem.GetGlobalNodeForElementNode(2))-mesh.GetNodePosition(elem.GetGlobalNodeForElementNode(1));
    
           A=(-l1.x()*l2.y()+l1.y()*l2.x())/2;
       for (int i=0; i<=2; ++i)
        {
            rhs[elem.GetGlobalNodeForElementNode(ie)]+=A*1./3.*eval_f(mesh.GetNodePosition(elem.GetGlobalNodeForElementNode(i)).x(),mesh.GetNodePosition(elem.GetGlobalNodeForElementNode(i)).y());//eval_f(b_x,b_y);
        }
