...
{
    if(laplaceProblem)  rhs=vector<float>(0.,rhs.size());
	for(size_t ie=0; ie<mesh.GetNumElements(); ie++) {
		const FEMElementTri& elem = mesh.GetElement(ie);
		//Task4 starts here
        for (int i=0;i<=2;++i)
        {
            rhs[ie]+=eval_f(mesh.GetNodePosition(GetGlobalNodeForElementNode(1)).x(),mesh.GetNodePosition(GetGlobalNodeForElementNode(1)).y());
